...
now that we've aggregated the data from the participating sites into one consolidated table, we'll need to modify the structure of the data and—in some cases—the inputted values, both to align the dataset with our analyses and to map site-provided responses with site-implemented practices

in addition, there are other reporting discrepancies that require standardization
...
here, we see that the respondent appended unit identifiers within the columns containing weight data. this will introduce NAs by coercion in the subsequent step, where we will convert columns containing numeric data from character strings to double-class vectors
  
to avoid this issue, we will remove these identifiers from the columns where they've been included 
...
of each (need to separate mint and cilantro) - may need to create item-specific variable set first 
